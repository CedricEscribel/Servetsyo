...
function HeaderTable($headerData)
{
    ...
    foreach ($headerData as $header) {
        $cellWidth = 22; // Set a fixed cell width (adjust as needed)
        $pdf->Cell($cellWidth, $cellHeight, $header, 1, 0, 'C');
    }
}

// Define the header data
$headerData = array('Full Name', 'Phone Number', 'Appointment', 'Animal' , 'Age', 'Breed', 'Gender', 'Date Requested', 'Set Date');


//code for print data
$sql = "SELECT  `FullN`, `PhoneNum`, `Sched`, `Animal`, `Age`, `Breed`,  `Gender`, `Date`, `SetDate` from  schedule ";
